...
public class PopupAdd extends JFrame {
	...
	private HashMap<Integer, ArrayList<Character>> passwords = new HashMap<Integer, ArrayList<Character>>(); // new reference to password for safety.
	...
	public void drawAdd(){
		...
		passwordField.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				String strength = PasswordGenerator.strengthConverter(PasswordGenerator.getPasswordStrength(
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				newid++; // new id is the highest id plus one. (ofc there is problem of overflow but idgaf)
				...
				model.insertRow(n, new String[]{ 
						Integer.toString(newid), 
						textApp.getText(), 
						textUsername.getText(), 
						String.format("%0" + passwords.get(newid).size() + "d", 0).replace("0","*") // create length times "*" string
					} 
				);
				dispose();
			}	
